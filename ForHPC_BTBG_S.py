...
def main(X):
    ...
    iddfile = path+ '/EP-8-9/EnergyPlus-8-9-0/Energy+.idd'
    IDF.setiddname(iddfile)
    epwfile = path+'/Final8Var3BGT/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
    ...
    fname1 = path +'/Final8Var3BGT/BT_BG_M.idf'
    epwfile = path+'/Final8Var3BGT/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
    ...
    idf.saveas(path +'/Final8Var3BGT/BT_BG_M.idf')
  
    #WWr
    from geomeppy import IDF
    
    fname2 = path +'/Final8Var3BGT/BT_BG_M.idf'
    idf1 = IDF(fname2,epwfile)
    idf1.set_wwr(wwr=0, wwr_map={180: i2}, force=True, construction= "Exterior Window")
    idf1.saveas(path +'/Final8Var3BGT/BT_BG_M.idf')
    ...
    for i in range (1,33):
        ...
        epwfile = path+'/Final8Var3BGT/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
        idf = IDF(fname1,epwfile)
        idf.saveas(path +'/Final8Var3BGT/BT_BG_M%d.idf'%(i))

        fnames.append(path +'/Final8Var3BGT/BT_BG_MS%d.idf'%(i))
    ...
    for i in range (1,33):
        Data=pd.read_csv(path +'/Final8Var3BGT/BT_BG_M%d.csv'%(i))
        ...
        file = path +'/Final8Var3BGT/BT_BG_M%dTable.csv'%(i)
        f = open(file,'rt')
        reader = csv.reader(f)
        csv_list = []
        for l in reader:
