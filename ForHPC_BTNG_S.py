...
def main(X):
    ...
    iddfile = path+ '/EP-8-9/EnergyPlus-8-9-0/Energy+.idd'
    IDF.setiddname(iddfile)
    epwfile = path+'/SenBTN/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
    ...
    fname1 = path +'/SenBTN/BT_NG_S.idf'
    epwfile = path+'/SenBTN/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
    ...
    for i in range (1,33):
        ...
        epwfile = path+'/SenBTN/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
        idf = IDF(fname1,epwfile)
        idf.saveas(path +'/SenBTN/BT_NG_S%d.idf'%(i))
        
        fnames.append(path +'/SenBTN/BT_NG_S%d.idf'%(i))
    ...
    for i in range (1,33):
        Data=pd.read_csv(path +'/SenBTN/BT_NG_S%d.csv'%(i))
        ...
        file = path +'/SenBTN/BT_NG_S%dTable.csv'%(i)
        f = open(file,'rt')
        reader = csv.reader(f)
        csv_list = []
        for l in reader:
    ...
    return np.average(TEUI)



# %%
a = np.arange(22,30.1,0.5)
Results_a = []
for i in a:
    Results_a=append(main(i))   
