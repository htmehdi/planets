...
def main(X):
    ...
    iddfile = path+ '/EP-8-9/EnergyPlus-8-9-0/Energy+.idd'
    IDF.setiddname(iddfile)
    epwfile = path+'/Final8Var2BTN/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
    ...
    fname1 = path +'/Final8Var2BTN/BT_NG_S.idf'
    epwfile = path+'/Final8Var2BTN/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
    ...
    idf.saveas(path +'/Final8Var2BTN/BT_NG_S.idf')
  
    #WWr
    from geomeppy import IDF
    
    fname2 = path +'/Final8Var2BTN/BT_NG_S.idf'
    idf1 = IDF(fname2,epwfile)
    idf1.set_wwr(wwr=0, wwr_map={180: i2}, force=True, construction= "Exterior Window")
    idf1.saveas(path +'/Final8Var2BTN/BT_NG_S.idf')
    #setting wshCTRL
    from eppy.modeleditor import IDF
    fname1 = path +'/Final8Var2BTN/BT_NG_S.idf'
    epwfile = path+'/Final8Var2BTN/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
    ...
    for i in range (1,33):
        ...
        epwfile = path+'/Final8Var2BTN/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
        idf = IDF(fname1,epwfile)
        idf.saveas(path +'/Final8Var2BTN/BT_NG_S%d.idf'%(i))
        
        fnames.append(path +'/Final8Var2BTN/BT_NG_S%d.idf'%(i))
    ...
    for i in range (1,33):
        Data=pd.read_csv(path +'/Final8Var2BTN/BT_NG_S%d.csv'%(i))
        ...
        file = path +'/Final8Var2BTN/BT_NG_S%dTable.csv'%(i)
        f = open(file,'rt')
        reader = csv.reader(f)
        csv_list = []
        for l in reader:
...
varbound= np.array([[0,16],[0,10],[0,10],[0,10],[0,8],[0,6],[0,3],[0,3],[0,10],[0,7],[0,4],[0,7],[1,120],[0,15],[0,10],[0,10],[0,10],[0,10],[0,3]])
