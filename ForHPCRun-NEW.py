...
def main(X):
    ...
    iddfile = path+ '/EP-8-9/EnergyPlus-8-9-0/Energy+.idd'
    IDF.setiddname(iddfile)
    epwfile = path+'/Final8Var2/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
    ...
    i2=i1[int(X[8])]
    #w = np.asscalar(X)
    fname1 = path +'/Final8Var2/M1Final3OCCSOV.idf'
    epwfile = path+'/Final8Var2/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
    ...
    # the second variable:
    occlightingmodel.Program_Line_3="set luxmean=" + str(b2)
    # the third variable:
    ...
    idf.saveas(path +'/Final8Var2/M1Final3OCCSOV.idf')

    #WWr
    from geomeppy import IDF
    
    fname2 = path +'/Final8Var2/M1Final3OCCSOV.idf'
    idf1 = IDF(fname2,epwfile)
    idf1.set_wwr(wwr=0, wwr_map={180: i2}, force=True, construction= "Exterior Window")
    idf1.saveas(path +'/Final8Var2/M1Fianl3OCCSOV.idf')
    ...
    for i in range (1,33):
        ...
        epwfile = path+'/Final8Var2/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
        idf = IDF(fname1,epwfile)
        idf.saveas(path +'/Final8Var2/M%dFinal3OCCSOV.idf'%(i))
        
        fnames.append(path +'/Final8Var2/M%dFinal3OCCSOV.idf'%(i))
    ...
    for i in range (1,33):
        Data=pd.read_csv(path +'/Final8Var2/M%dFinal3OCCSOV.csv'%(i))
        ...
        TON.append(ON)
        TOFF.append(OFF)
        f = open(path+'/Final8Var2/M%dFinal3OCCSOVTable.csv'%(i))
        reader = csv.reader(f)
        csv_list = []
        for l in reader:
            csv_list.append(l)
...
varbound= np.array([[1,120],[0,15],[0,10],[0,10],[0,10],[0,10],[0,3],[0,3],[0,10]])
