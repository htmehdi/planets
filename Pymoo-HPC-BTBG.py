...
class MultiNGBT(Problem):
    ...
    def __init__(self):
        super().__init__(n_var=7, n_obj=2, n_constr=0, elementwise_evaluation=True, type_var=int)
        #self.args = args
        self.xl=anp.array([0,0,0,0,0,0,0])
        self.xu = anp.array([3,10,10,10,10,10,3])        
    ...
    def _evaluate(self, X, out,*args, **kwargs):
        ...
        iddfile = path+ '/EP-8-9/EnergyPlus-8-9-0/Energy+.idd'
        IDF.setiddname(iddfile)
        epwfile = path+'/PMBTBG/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
        ...
        """  
        fname1 = path +'/PMBTBG/BT_BG_To_T_L.idf'
        epwfile = path+'/PMBTBG/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
        ...
        idf.saveas(path +'/PMBTBG/BT_BG_To_T_L.idf')

        #WWr
        from geomeppy import IDF

        fname2 = path +'/PMBTBG/BT_BG_To_T_L.idf'
        idf1 = IDF(fname2,epwfile)
        idf1.set_wwr(wwr=0, wwr_map={180: i2}, force=True, construction= "Exterior Window")
        idf1.saveas(path +'/PMBTBG/BT_BG_To_T_L.idf')
        #setting wshCTRL
        from eppy.modeleditor import IDF
        fname1 = path +'/PMBTBG/BT_BG_To_T_L.idf'
        epwfile = path+'/PMBTBG/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
        ...
        for i in range (1,4):
            fname1 = path +'/PMBTBG/BT_BG_To_T_L.idf'
            epwfile = path+'/PMBGBT/CAN_PQ_Montreal.Intl.AP.716270_CWEC.epw'
            idf = IDF(fname1,epwfile)
            idf.saveas(path +'/PMBTBG/BT_BG_To_T_L%d.idf'%(i))

            fnames.append(path +'/PMBTBG/BT_BG_To_T_L%d.idf'%(i))
        ...
        for i in range (1,4):
            Data=pd.read_csv(path +'/PMBTBG/BT_BG_To_T_L%d.csv'%(i))
            ...
            file = path +'/PMBTBG/BT_BG_To_T_L%dTable.csv'%(i)
            f = open(file,'rt')
            reader = csv.reader(f)
            csv_list = []
            for l in reader:
